...
module eom
  ...
  real(dl), parameter :: omega = 50._dl*2._dl*nu**0.5, del = (nu/2._dl)**0.5*(1._dl+0.3_dl)
  real(dl), parameter :: len = 50._dl / (2.*nu)**0.5, dx = len/dble(nLat), dk = twopi/len
  real(dl), parameter :: rho = 2.**(-3)*1000._dl*2._dl*(nu)**0.5
